...
export default function Thesis() {
  ...
  const s = application !== undefined && application
    ? (STATUS[application.status] ?? { label: application.status, bg: '#f1f3f6', color: '#4d6080' })
    : null

  const chatPrefillActive = application?.status === 'ACTIVE'
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ fontSize: '0.875rem', color: '#8fa3c4', margin: '0.2rem 0 0 0' }}>
          Supervision application and consultation history.
        </p>
      </div>

      {loadError && <p style={{ fontSize: '0.85rem', color: '#c0392b', marginBottom: '0.75rem' }}>{loadError}</p>}
      {profError && !application && <p style={{ fontSize: '0.85rem', color: '#c0392b', marginBottom: '0.75rem' }}>{profError}</p>}
      ...
      {application ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
          ...
          {/* Mentor card */}
          <div style={{ background: '#fff', border: '1px solid #e8ecf0', borderTop: '3px solid #2a9960', borderRadius: 12, padding: '1rem 1.15rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', flexWrap: 'wrap' }}>
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: '0.68rem', fontWeight: 600, color: '#8fa3c4', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.3rem 0' }}>
                  {application.status === 'ACTIVE' ? 'Mentor' : 'Applied to'}
                </p>
                <p style={{ fontWeight: 600, fontSize: '1rem', color: '#0f1f3d', margin: 0 }}>{application.professor_name}</p>
                ...
                <p style={{ fontSize: '0.75rem', color: '#aab8cc', margin: '0.35rem 0 0 0' }}>
                  Applied {new Date(application.applied_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              </div>
              {s && (
                <span style={{ flexShrink: 0, fontSize: '0.75rem', fontWeight: 600, padding: '0.25rem 0.7rem', borderRadius: 20, background: s.bg, color: s.color }}>
                  {s.label}
                </span>
              )}
            </div>
          ...
          {application.status === 'REJECTED' && (
            <div style={{ background: '#fff5f5', border: '1px solid #ffc9c9', borderRadius: 10, padding: '0.85rem 1rem', fontSize: '0.85rem', color: '#c0392b' }}>
              Your application was declined. You can apply to another professor below or through the booking chat.
            </div>
          )}

          {application.status === 'PENDING' && (
            <div style={{ background: '#fffbf0', border: '1px solid #f5e6c0', borderRadius: 10, padding: '0.85rem 1rem', fontSize: '0.85rem', color: '#92570a', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap' }}>
              <span>Under review — once approved you can book follow-up sessions through the chat.</span>
              <button
                type="button"
                style={{ padding: '0.3rem 0.75rem', borderRadius: 6, border: '1px solid #f5d4a0', background: '#fff', fontSize: '0.8rem', fontWeight: 500, color: '#92570a', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}
          ...
          {err && <p style={{ fontSize: '0.85rem', color: '#c0392b', margin: 0 }}>{err}</p>}

          {/* Active: mentor info + history */}
          {application.status === 'ACTIVE' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: '1.25rem', alignItems: 'start' }}>

              {/* Left: thesis consultation hours + chat link */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', minWidth: 0 }}>
                <p style={{ fontSize: '0.85rem', color: '#2d6a4f', margin: 0, lineHeight: 1.5 }}>
                  Thesis supervision is active — book follow-ups through the chat below.
                </p>
                {mentor && (mentor.consultation_thesis_hours?.length ?? 0) > 0 && (
                  <div style={{ background: '#fff', border: '1px solid #e8ecf0', borderRadius: 10, padding: '0.85rem 1rem' }}>
                    <p style={{ fontSize: '0.68rem', fontWeight: 600, color: '#8fa3c4', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.4rem 0' }}>
                      Mentor thesis hours
                    </p>
                    <ul style={{ margin: 0, paddingLeft: '1.1rem', fontSize: '0.82rem', color: '#4d6080', lineHeight: 1.6 }}>
                ...
                <Link
                  ...
                  style={{ display: 'inline-block', alignSelf: 'flex-start', background: '#1a2744', color: '#fff', padding: '0.55rem 1.1rem', borderRadius: 8, fontSize: '0.85rem', fontWeight: 600, textDecoration: 'none' }}
                >
                  Open booking chat
                </Link>
              </div>
              ...
              <div style={{ minWidth: 0 }}>
                <p style={{ fontSize: '0.68rem', fontWeight: 600, color: '#8fa3c4', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 0.65rem 0' }}>
                  Thesis consultation history
                </p>
                {historyLoading && <p style={{ fontSize: '0.82rem', color: '#aab8cc' }}>Loading history…</p>}
                {(historyError || historyActionErr) && (
                  <p style={{ fontSize: '0.82rem', color: '#c0392b', marginBottom: '0.5rem' }}>{historyActionErr ?? historyError}</p>
                )}
                {!historyLoading && thesisHistory.length === 0 && (
                  <div style={{ background: '#fff', border: '1px dashed #d1d9e6', borderRadius: 10, padding: '0.85rem 1rem', fontSize: '0.875rem', color: '#6b7ea8' }}>
                    No thesis consultations yet.
                  </div>
                )}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '0.85rem' }}>
                  {thesisHistory.map((r: ThesisConsultationHistoryRow) => {
                    const chip = historyStatusChip(r.status)
                    const day = r.session_date ? new Date(r.session_date) : null
                    if (day) day.setHours(0, 0, 0, 0)
                    const isUpcoming = r.status === 'ACTIVE' && day != null && day >= today
                    const topic = (r.task ?? '').trim() || null
                    ...
                    const footer = (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                        {isUpcoming && (
                          <button
                            type="button"
                            style={{ flex: 1, padding: '0.35rem 0.5rem', border: '1px solid #ffc9c9', borderRadius: 6, background: '#fff5f5', fontSize: '0.74rem', fontWeight: 600, color: '#c0392b', cursor: 'pointer' }}
                            onClick={async () => {
                        ...
                        {r.status === 'ATTENDED' && !r.has_feedback && (
                          <button
                            type="button"
                            style={{ flex: 1, padding: '0.35rem 0.5rem', border: '1px solid #d1d9e6', borderRadius: 6, background: '#fff', fontSize: '0.74rem', fontWeight: 600, color: '#1a2744', cursor: 'pointer' }}
                        ...
                        {r.status === 'ATTENDED' && r.has_feedback && (
                          <span style={{ fontSize: '0.74rem', color: '#1a7a4a', fontWeight: 600, padding: '0.35rem 0.5rem' }}>
                            Feedback sent
                          </span>
                        )}
                      </div>
                    ...
                    return (
                      <BookingCard
                        ...
                        primaryLabel="Mentor"
                        primaryName={application.professor_name}
                        statusBadges={chip ? [chip] : []}
                        subject={r.course_code && r.course_name ? `${r.course_code} · ${r.course_name}` : (r.course_name ?? null)}
                        hall={r.hall}
                        topic={topic}
        ...
        <div style={{ maxWidth: 520 }}>
          <p style={{ fontSize: '0.875rem', color: '#6b7ea8', marginBottom: '1.25rem', lineHeight: 1.5 }}>
            No active application. Apply below — this form and the booking chat use the same queue.
          </p>

          {!eligible && (
            <div style={{ background: '#fffbf0', border: '1px solid #f5e6c0', borderRadius: 10, padding: '0.85rem 1rem', marginBottom: '1rem', fontSize: '0.85rem', color: '#92570a' }}>
              Thesis applications are limited to final-year students. Contact the faculty office if you believe this is a mistake.
            </div>
          )}

          {err && <p style={{ fontSize: '0.85rem', color: '#c0392b', marginBottom: '0.75rem' }}>{err}</p>}
          ...
          {eligible && (
            <p style={{ fontSize: '0.78rem', color: '#aab8cc', marginTop: '1rem' }}>
              Prefer chat?{' '}
              <Link to="/student/chat" state={{ prefill: 'I want to apply for thesis supervision' }} style={{ color: '#3b5bdb' }}>
                Open booking chat
              </Link>
            </p>
          )}
      ...
      {feedbackId !== null && (
        <FeedbackModal
          ...
          onClose={() => { setFeedbackId(null); void reloadThesisHistory() }}
        />
      )}
    </div>
  )
